...
class Tests {
    ...
    fun russian() {
        ...
        assertEquals("тридцать", russian(30))
        assertEquals("сто", russian(100))
        assertEquals("двести", russian(200))
        assertEquals("тысяча", russian(1000))
        ...
        assertEquals("сто восемьдесят одна тысяча двести девятнадцать", russian(181219))
        assertEquals("восемьсот пятнадцать тысяч четыреста девятнадцать", russian(815419))
        assertEquals("пятьсот две тысячи сто семьдесят четыре", russian(502174))
        assertEquals("сто двадцать тысяч один", russian(120001))
        assertEquals("восемьсот девяносто тысяч пятьсот семьдесят шесть", russian(890576))
        assertEquals("десять", russian(10))
        assertEquals("четыреста восемьдесят пять тысяч семьсот девяносто шесть", russian(485796))
        assertEquals("девятнадцать тысяч двести сорок четыре", russian(19244))
        assertEquals("двадцать тысяч", russian(20000))
        assertEquals("сто тысяч", russian(100000))
        assertEquals("семьсот двенадцать тысяч триста двадцать три", russian(712323))
        assertEquals("двадцать восемь тысяч триста сорок девять", russian(28349))
        assertEquals("пятьдесят тысяч семьсот пятьдесят шесть", russian(50756))
        assertEquals("сто четыре тысячи шестьсот шестьдесят пять", russian(104665))
        assertEquals("сто десять тысяч пятьсот", russian(110500))
